...
def login(conn, **extra_opt):
    ...
    auth_client_object = _pam_interactive_ClientAuthState_patched(conn, depot, scheme=PAM_INTERACTIVE_SCHEME)
    auth_client_object.authenticate_client(
        initial_request=extra_opt
    )

class _pam_interactive_ClientAuthState_patched(_pam_interactive_ClientAuthState):
    ...
    def auth_client_start(self, request):
        ...
        if storage and os.path.isfile(storage) and os.access(storage, os.R_OK):
            with io.open(storage, 'r') as storage:
    ...
    def authenticated(self, request):
        ...
        storage = getattr(self.conn.account, "pstate_storage", None)
        if storage:
            with os.fdopen(os.open(storage, os.O_WRONLY | os.O_CREAT, 0o600), 'w') as storage:
